...
export default function DashboardAssistant({ contextJson, className }: Props) {
  ...
  const persistActiveThread = useCallback(
    (id: string) => {
      if (!user?.id) return
      try {
        localStorage.setItem(assistantActiveThreadKey(user.id), id)
      } catch {
        /* ignore */
      }
    },
  ...
  useEffect(() => {
    ...
    if (!supabase) {
      ...
      setTurns(loadAssistantTurns(user.id))
      return
    }
    ...
    void (async () => {
      try {
        ...
        if (list.length === 0) {
          const local = loadAssistantTurns(user.id)
          if (local.length > 0) {
            const t = await createAssistantThread(supabase, user.id, 'Conversas importadas')
            await bulkInsertAssistantMessages(supabase, t.id, local)
            clearAssistantTurns(user.id)
        ...
        try {
          stored = localStorage.getItem(assistantActiveThreadKey(user.id))
        } catch {
          stored = null
        }
        const pick =
          stored && list.some((x) => x.id === stored) ? stored : (list[0]?.id ?? null)
        if (!cancelled && pick) {
          setThreads(list)
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      saveAssistantTurns(user.id, turns)
    }, 400)
    return () => {
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current)
    }
  }, [user?.id, configured, turns, supabase])
  ...
  const send = useCallback(
    ...
    [contextJson, configured, loading, turns, user?.id, supabase, activeThreadId, threads]
  )
    ...
    <div className={cn('flex min-h-0 flex-1 flex-col border-[#d4d2c8] bg-white', className)}>
      <div className="flex shrink-0 items-center gap-2 border-b border-[#d4d2c8] px-3 py-2 sm:px-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-brand-dark">Assistente</p>
      ...
      {!configured ? (
        <div className="flex-1 space-y-2 p-4 text-sm text-muted-foreground">
          <p>
            Com sessão ativa, o app chama a Edge Function <code className="rounded bg-muted px-1 text-xs">openrouter-chat</code>{' '}
            (segredo <code className="rounded bg-muted px-1 text-xs">OPENROUTER_API_KEY</code> no Supabase).
          </p>
          <p className="text-[11px]">Em desenvolvimento local, pode usar no <code className="text-xs">client/.env</code>:</p>
          <pre className="overflow-x-auto rounded-md border bg-muted/40 p-2 text-[11px]">
            VITE_OPENROUTER_API_KEY=sk-or-...
            {'\n'}
            VITE_OPENROUTER_MODEL=openai/gpt-4o-mini
          </pre>
          <p className="text-[11px]">A chave <code className="text-xs">VITE_*</code> vai no bundle; prefira a função em produção.</p>
        </div>
      ) : (
        <div
          className={cn(
